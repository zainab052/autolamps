function openerp_pos_printing_autolamps_widgets(instance, module) {
  ...
  module.PaymentScreenWidget = module.PaymentScreenWidget.extend({
    ...
    validate_order: function (options) {
      ...
      for (var i = 0; i < plines.length; i++) {
        if (plines[i].get_type() === "bank" && plines[i].get_amount() < 0) {
          this.pos_widget.screen_selector.show_popup("error", {
            ...
            comment: _t(
              "You cannot have a negative amount in a Bank payment. Use a cash payment method to return money to the customer."
            ),
          });
          return;
        }
      ...
      if (
        Math.abs(
          currentOrder.getTotalTaxIncluded() - currentOrder.getPaidTotal()
        ) > 0.00001
      ) {
        var cash = false;
      ...
      if (options.invoice) {
        ...
        invoiced.fail(function (error) {
          if (error === "error-no-client") {
            self.pos_widget.screen_selector.show_popup("error", {
              ...
              comment: _t(
                "Please select a client for this order. This can be done by clicking the order tab"
              ),
            });
          } else {
            self.pos_widget.screen_selector.show_popup("error", {
        ...
        if (this.pos.config.iface_print_via_proxy) {
          ...
          this.pos.proxy.print_qz_tofile(receipt);
          this.pos.get("selectedOrder").destroy(); //finish order and go back to scan screen
        } else {
          // var receipt = currentOrder.export_for_printing();

          // print_job(receipt, self);
  ...
  module.ReceiptScreenWidget = module.ScreenWidget.extend({
    ...
    print: function () {
      ...
      var order_widget = $(".pos-receipt-container").prop("outerHTML");
      console.log(order_widget);
      formatedReceipt(receipt, context);
    },
    ...
    refresh: function () {
      var order = this.pos.get("selectedOrder");
      $(".pos-receipt-container", this.$el).html(
  ...
  const fetchEsd = async (esdOption) => {
  ...
  // async function print_job(receiptData, context) {
  //     var self = context
  //     var printer_name = self.pos.config.printer_name;
  //     var esd_option = self.pos.config.esd_option
  //     console.log(receiptData, context);
  //     data = await formatedReceipt(receiptData,context)

  //     var config = qz.configs.create(printer_name,{ copies: 2 });
  ...
  function startConnection(config) {
    if (!qz.websocket.isActive()) {
      ...
      displayMessage(
        "An active connection with QZ already exists.",
        "alert-warning"
      );
    }
  }
  ...
  function findPrinters() {
    qz.printers
      ...
      .then(function (data) {
        ...
        displayMessage(
          "<strong>Available printers:</strong><br/>" + list,
          null,
          15000
        );
      })
          ...
          findConnection: function (config, resolve, reject) {
            ...
            if (!config.port.secure.length) {
              if (!config.port.insecure.length) {
                reject(
                  new Error("No ports have been specified to connect over")
                );
                return;
                    ...
                    try {
                      if (
                        ...
                        _qz.websocket.connection.semver[
                          i
                        ] = -_qz.websocket.connection.semver[i].replace(
                          /\D/g,
                          ""
                        );
                        continue;
          ...
          for (var i = 0; i < data.length; i++) {
            if (data[i].constructor === Object) {
              ...
              if (data[i].data.search(/data:image\/\w+;base64,/) === 0) {
                ...
                if (
                  ["FILE", "XML"].indexOf(data[i].flavor.toUpperCase()) > -1
                ) {
                  absolute = true;
                }
              } else if (
                ...
                ((["PIXEL", "IMAGE", "PDF"].indexOf(
                  ...
                  (["HTML", "PDF"].indexOf(data[i].type.toUpperCase()) > -1 &&
                    (!data[i].format ||
                      data[i].format.toUpperCase() === "FILE")))
              ) {
      ...
      SHA: {
        ...
        hash: function (msg) {
          ...
          for (var i = 0; i < N; i++) {
            ...
            for (var t = 0; t < 64; t++) {
              var T1 =
                h + _qz.SHA._sig1(e) + _qz.SHA._ch(e, f, g) + K[t] + W[t];
    ...
    var qz = {
      ...
      security: {
        ...
        setSignatureAlgorithm: function (algorithm) {
          ...
          if (
            ["SHA1", "SHA256", "SHA512"].indexOf(algorithm.toUpperCase()) < 0
          ) {
            _qz.log.error(
  ...
  const PAD_BOTH = "both";
  async function formatedReceipt(data, context) {
    let self = context;
    var printer_name = self.pos.config.printer_name;
    var esd_option = self.pos.config.esd_option;
    var config = qz.configs.create(printer_name, { copies: 1 });
    let esd;
    // await fetchEsd(true).then((response) => {
    //   esd = response.text();
    // }).catch((error) => {
    //     esd =""
    ...
    //   });
    try {
      const response = await fetchWithTimeout("http://192.168.1.174/esd", {
        timeout: 10000,
      });
      esd = await response.text();
    } catch (error) {
      esd = "";
      console.log(error);
    }
    const TopImage = {
      ...
      format: "image",
      flavor: "file",
      data: "/pos_printing_autolamps/static/src/assets/img/Autolamps.png",
      options: { language: "ESCPOS", dotDensity: "double", colorType: "color" },
    };
    const BottomImage = {
      type: "raw",
      format: "image",
      flavor: "file",
      data: "/pos_printing_autolamps/static/src/assets/img/Autoglass Logo.png",
      options: { language: "ESCPOS", dotDensity: "double", colorType: "color" },
    };
    ...
    chunk.push(lineBreake);
    chunk.push("39 Kirinyaga Road, Merchandise Centre" + lineBreake);
    chunk.push("P.O. BOX 6741-00100, Nairobi, Kenya" + lineBreake);
    chunk.push("Tel: 0751 999500 / 580" + lineBreake);
    chunk.push("Email: sales@autolampskenya.com" + lineBreake);
    chunk.push("WWW.AUTOLAMPSKENYA.COM" + lineBreake);
    chunk.push("PIN: P051363500N" + lineBreake);
    chunk.push("MPESA PAYBILL: 182754" + lineBreake);
    chunk.push(lineBreake);
    chunk.push(
      "Customer:" + (data.client != null ? data.client : "") + lineBreake
    );
    chunk.push(lineBreake);
    chunk.push(alignLeft);
    chunk.push(
      `Cash Sale No:${data.name}       ` +
        "  " +
        `Date: ${data.date.localestring}`
    );
    chunk.push(lineBreake);
    // chunk.push(boldOn)
    // chunk.push(emMode)
    ...
    chunk.push(boldOn);
    chunk.push(properties(data));

    chunk.push(boldOff);
    chunk.push(lineBreake);
    chunk.push(paymentTypes(data));
    chunk.push(lineBreake);
    chunk.push(line);
    chunk.push(alignCenter);
    ...
    chunk.push("SIGN  --------------------------");
    chunk.push(lineBreake);
    chunk.push("GOODS RECEIVED IN GOOD CONDITION");
    chunk.push(lineBreake);
    chunk.push(lineBreake);
    chunk.push(alignCenter);
    chunk.push(boldOn);
    chunk.push(
      `GOODS ONCE SOLD CANNOT BE RETURNED.${lineBreake}PLEASE CHECK YOUR GOODS PROPERLY.${lineBreake}PLEASE VISIT US AGAIN.${lineBreake}THANK YOU.`
    );
    chunk.push(boldOff);
    ...
    //   chunck.push(comapanyDetail)
    // })
    await qz.print(config, chunk).catch(function (e) {
      console.error(e);
    });
    chunk.length = 0;
  }
  ...
  const header = () => {
    ...
    headerLine.push(
      [
        pad("Item Name", " ", 36, PAD_RIGHT),
        pad("Qty", " ", 6, PAD_RIGHT),
        pad("Price", " ", 2, PAD_RIGHT),
        // pad('KES', 10, PAD_RIGHT),
        pad("Total", " ", 8, PAD_LEFT),
      ].join("")
    );
    return headerLine.join(SpaceType);
  };
  const txLines = (lines) => {
    let trailing = "";
    let name = "";
    ...
    lines.forEach(function (line) {
      if (line.product_name.length > 26) {
        trailing = line.product_name.slice(26);
        name = line.product_name.slice(0, 26);
        orderlines.push(
          [
            pad(name, " ", 36, PAD_RIGHT),
            pad(line.quantity, " ", 6, PAD_RIGHT),
            pad(line.price.format(0), " ", 2, PAD_RIGHT),
            // pad('KES', 10, PAD_RIGHT),
            pad(line.price_display.format(0), " ", 8, PAD_LEFT),
            lineBreake,
            trailing,
            lineBreake,
          ].join("")
        ...
        // chunk.push(alignLeft)
        // chunk.push(trailing + lineBreake)
      }
    ...
    chunk = chunk.concat(orderlines);
  };
  const properties = (data) => {
    let widest = 0;
    let lines = [];
    let paymentlines = [
      { name: "TOTAL INC VAT 16%", value: data.total_with_tax },
      { name: "TOTAL EXC VAT 16%", value: data.total_without_tax },
      { name: "VAT 16%", value: data.total_tax },
      { name: "TOTAL PAID", value: data.total_paid },
      { name: "CHANGE", value: Math.abs(data.change) },
    ];
    for (let line of paymentlines) {
      lines.push(
        [
          pad(line.name, " ", 47, PAD_RIGHT),
          // utils.pad('Product', ' ', this.config.width - 18, utils.PAD_RIGHT),
          pad(line.value.format(0), " ", 8, PAD_LEFT),
        ].join("")
      );
    }
  ...
  const paymentTypes = (data) => {
    ...
    for (let line of data.paymentlines) {
      lines.push(
        [
          pad(line.journal, " ", 47, PAD_RIGHT),
          // utils.pad('Product', ' ', this.config.width - 18, utils.PAD_RIGHT),
          pad(line.amount.format(0), " ", 8, PAD_LEFT),
        ].join("")
      );
    }
    return lines.join(SpaceType);
