...
class Settlement(models.Model):
    ...
    @api.depends('agent_target', 'sales_over_target')
    def _compute_payable_factor(self):
        for record in self:
            if record.agent_target > 0:
                record.payable_factor = (record.sales_over_target / record.agent_target)
